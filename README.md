...
| **2026-04-25** | v1.7 | Redesigned **booking Step 3 seat selection** with a cleaner aircraft-style seat map: 3+3 cabin layout (A-F), visible aircraft body, nose, tail, larger wing silhouette, free seat selection, clear Available/Taken/Selected legend, and a right-side passenger summary panel showing the selected seat. Removed paid/recommended seat-section logic from the UI so seats remain free. Improved seat selection JavaScript so the selected-seat badge updates live and Space no longer toggles seats. Strengthened **race condition protection** for simultaneous seat reservations: booking now locks affected Flight rows, validates one seat per passenger, checks only active booked tickets, and the database now enforces a partial unique index for `(flight_id, seat_number)` on booked tickets via Flyway migration V11 plus a matching Django model constraint. |
| **2026-04-28** | v1.8 | Completed **Phase 3 design work**: added Destinations page (Leaflet map with CARTO tiles, country/city cards with flag-icons library, 2×2 photo gallery), Timetable page (filterable by departure city), Seat Classes comparison page, and a shared 3-column info footer added to all public pages. Created info pages: Travel Documents, Baggage, Special Categories, Help (FAQ accordion), Special Offers (random upcoming flights), About the Company, and Media & News (6 articles with images). Upgraded home page with a stats strip (routes/countries/passengers/since), "Why fly with us" 4-card grid, Latest News section with real images, and two clickable info cards (Seat Reservation, Extra Luggage). Flight search page enriched with tips strip, Popular Routes grid (random, 5×2), and Cheapest Upcoming Flights cards (random). Multi-currency support extended to all new pages and PDF/email receipts. Wired all hamburger menu links to new pages. **Responsive design** implemented across all CSS files — mobile-first rules for nav, search dropdowns (full-screen overlay on mobile with close button), flight result tables (horizontal scroll), booking Step 2 class comparison table, info page grids, timetable, seat classes, footer, and home page sections. Extracted all remaining inline JavaScript to external static files (`faq.js`, `timetable.js`, `flight_list_search.js`); moved all hardcoded inline styles from `base.html` to `nav.css`. |
| **2026-04-30** | v1.9 | **Code simplification and cleanup**: extracted `require_booking_session` decorator (applied to booking steps 2–5), `staff_required` decorator (applied to all admin views), and `_base_total()` helper to eliminate repeated expressions. Moved `get_return_flight()` into `BookingSession` as a method. Extracted `_style_password_form()` in profile view. Simplified `verify_checkin_data` (removed 3 intermediate boolean variables), `all_selected_seats` loop (switched to `setdefault`), and `build_seat_positions` (removed dead `get_price_label` function). Fixed duplicate `active_flights` query and duplicate context key in admin panel. Removed duplicate `Sarajevo` entry in `CITY_COORDS`. Merged split `django.shortcuts` imports. Removed all Python comments, docstrings, CSS comments, and HTML comments across the entire codebase. Fixed half-screen responsive breakpoint for search widget and promo cards (new `1100px–769px` media query). |
| **2026-05-01** | v1.10 | **Refactor/simplifier update**: moved shared access checks into `decorators.py` (`staff_required`, `require_booking_session`) and reduced repeated logic across admin and booking views. Simplified the booking flow with `_parse_seat_picks`, cleaner session access, internal total price calculation, and a smaller PayPal amount setup. Centralized email building/sending with shared `_h1()` and `_send()` helpers across receipt, password, cancellation, reminder, and check-in emails. Lazy-loaded WeasyPrint inside PDF generation so management commands like `seed_routes` can run even when Windows PDF native libraries are missing. Reused `future_flights_q()` across dashboard/info pages, cleaned ticket filters with model status constants, simplified occupied-seat handling in the seat map, merged duplicate imports, and skipped scheduler startup for `seed_routes`. |
...
### Phase 4 — 02.05 → 08.05
- **Polish of code and fixes** — final cleanup, bug fixes and presentation preparation
