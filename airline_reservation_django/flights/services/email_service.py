...
def _h1(title, subtitle=None, size=24):
    sub = (
        f'<p style="margin:10px 0 0;color:rgba(255,255,255,0.55);font-size:13px;font-family:{SANS};">{subtitle}</p>'
        if subtitle else ''
    )
    return (
        f'<h1 style="margin:8px 0 0;color:{_C["white"]};font-family:{FONT};font-size:{size}px;font-weight:600;font-style:italic;">{title}</h1>'
        f'{sub}'
    )


def _send(to_email, subject, text_body, html_body, label, pdf_buffer=None):
    try:
        ...
        print(f"[EMAIL ERROR] Failed to send {label} email to {to_email}: {e}")
        return False
...
def send_receipt_email(to_email, total_sum, pdf_buffer=None, flight=None, user=None, currency='EUR'):
    ...
    header_html = _h1("Booking Confirmed", "Your seat is reserved. Safe travels!", size=26)
    body_html = f"""
    ...
    return _send(to_email, subject, text_body, _wrap(header_html, body_html), "receipt", pdf_buffer)
...
def send_password_changed_email(to_email, username):
    ...
    header_html = _h1("Password Changed")
    body_html = f"""
    ...
    return _send(to_email, subject, text_body, _wrap(header_html, body_html, _C['navy']), "password changed")
...
def send_flight_canceled_email(to_email, passenger_name, flight):
    ...
    header_html = _h1("Flight Canceled", "We apologize for the inconvenience")
    body_html = f"""
    ...
    return _send(to_email, subject, text_body, _wrap(header_html, body_html, _C['danger']), "flight canceled")
...
def send_ticket_canceled_email(to_email, passenger_name, flight, ticket):
    ...
    header_html = _h1("Ticket Canceled", "Your refund has been confirmed")
    body_html = f"""
      <p style="margin:0 0 6px;font-size:16px;color:{_C['text']};font-family:{SANS};">Dear <strong>{passenger_name}</strong>,</p>
      <p style="margin:0 0 28px;font-size:14px;color:{_C['muted']};font-family:{SANS};line-height:1.6;">
    ...
    return _send(to_email, subject, text_body, _wrap(header_html, body_html, _C['success']), "ticket canceled")
...
def send_flight_reminder_email(to_email, passenger_name, flight):
    ...
    header_html = _h1("Flight in 24 Hours", "Time to get ready for your journey")
    body_html = f"""
      <p style="margin:0 0 6px;font-size:16px;color:{_C['text']};font-family:{SANS};">Dear <strong>{passenger_name}</strong>,</p>
      <p style="margin:0 0 28px;font-size:14px;color:{_C['muted']};font-family:{SANS};line-height:1.6;">
    ...
    return _send(to_email, subject, text_body, _wrap(header_html, body_html, _C['accent']), "flight reminder")
...
def send_checkin_email(to_email, flight_number):
    ...
    header_html = _h1("Check-in Confirmed")
    body_html = f"""
      <p style="margin:0 0 24px;font-size:14px;color:{_C['muted']};font-family:{SANS};line-height:1.6;">
    ...
    return _send(to_email, subject, text_body, _wrap(header_html, body_html, _C['success']), "check-in")
